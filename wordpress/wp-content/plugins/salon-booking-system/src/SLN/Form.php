    ...
    static public function fieldJSTime($name, $value, $settings){ 
       ...
       $phpFormat = SLN_Enum_TimeFormat::getPhpFormat($f);
        ...
        <div class="sln_timepicker"><input type="text" name="<?php echo $name ?>" id="<?php echo self::makeID($name) ?>" 
            ...
            data-popup-class="<?php echo (isset($settings['popup-class']) ? $settings['popup-class'] : '') ?>"
            data-work-times="<?php echo esc_attr(json_encode(SLN_Plugin::getInstance()->getAvailabilityHelper()->getItems()->getTimeMinMax())) ?>"/></div>
        </span><?php
    }
