...
if (!class_exists("nxs_class_SNAP_VK")) { class nxs_class_SNAP_VK {
    ...
    function nxs_uplImgtoVK($imgURL, $options){
      $postUrl = 'https://api.vk.com/method/photos.getWallUploadServer?v=5.71&gid='.(str_replace('-','',$options['pgIntID'])).'&access_token='.$options['appAuthToken'];
      ...
      if (!empty($VKuploadUrl)) {    
        ...
        if (!empty($uploadResultObj->server) && !empty($uploadResultObj->photo) && !empty($uploadResultObj->hash)) {
          $postUrl = 'https://api.vk.com/method/photos.saveWallPhoto?v=5.71&server='.$uploadResultObj->server.'&photo='.$uploadResultObj->photo.'&hash='.$uploadResultObj->hash.'&gid='.(str_replace('-','',$options['pgIntID'])).'&access_token='.$options['appAuthToken'];
    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>''); $atts = array(); //prr($message); die();
      ...
      if($postType=='A') $atts[] = $urlToGo; if (is_array($atts)) $atts = implode(',', $atts); $postUrl = 'https://api.vk.com/method/wall.post?v=5.71'; $msg = strip_tags($msg);
      $postArr = array('owner_id'=>$options['pgIntID'], 'access_token'=>$options['appAuthToken'], 'from_group'=>'1', 'message'=>$msg, 'attachments'=>$atts, 'v'=>'5.71');
